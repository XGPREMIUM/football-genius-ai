...
export default function Home() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-950 text-gray-100 selection:bg-amber-500/30">
      {/* Sticky Header */}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="flex items-center h-14 gap-4">
            <button onClick={() => { setShowChat(false); setMessages([]) }} className="flex items-center gap-2 shrink-0 group">
              ...
              <span className="font-bold text-sm tracking-tight text-gray-100 group-hover:text-white transition-colors hidden sm:inline">{t.title}</span>
            </button>
            <nav className="hidden md:flex items-center gap-1 flex-1 overflow-x-auto py-1">
              {NAV_MODES.map(mid => (
                <button key={mid} onClick={() => setMode(mid)}
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all whitespace-nowrap ${mode === mid ? "bg-amber-500/10 text-amber-400" : "text-gray-400 hover:text-gray-200 hover:bg-gray-800/50"}`}>
            ...
            <div className="flex items-center gap-2 ml-auto">
              <div className="flex items-center bg-gray-800/50 rounded-lg p-0.5 border border-gray-700/50">
                <button onClick={() => setLang("es")} className={`px-2 py-1 rounded-md text-xs font-medium transition-all ${lang === "es" ? "bg-amber-500 text-black" : "text-gray-400 hover:text-gray-200"}`}>ES</button>
                <button onClick={() => setLang("en")} className={`px-2 py-1 rounded-md text-xs font-medium transition-all ${lang === "en" ? "bg-amber-500 text-black" : "text-gray-400 hover:text-gray-200"}`}>EN</button>
              </div>
              {showChat && messages.length > 0 && (
                <div className="flex items-center gap-1">
                  <button onClick={shareChat} className="p-1.5 rounded-lg text-xs text-gray-400 hover:text-gray-200 hover:bg-gray-800/50 transition-all" title={t.share}>📤</button>
                  <button onClick={copyChat} className="p-1.5 rounded-lg text-xs text-gray-400 hover:text-gray-200 hover:bg-gray-800/50 transition-all" title={t.copied}>{copied ? "✅" : "📋"}</button>
                  <button onClick={exportChat} className="p-1.5 rounded-lg text-xs text-gray-400 hover:text-gray-200 hover:bg-gray-800/50 transition-all" title="Exportar">📥</button>
                  <button onClick={() => { setMessages([]); setShowChat(false); speechSynthesis.cancel() }} className="p-1.5 rounded-lg text-xs text-gray-400 hover:text-gray-200 hover:bg-gray-800/50 transition-all" title={t.newChat}>✕</button>
                </div>
              )}
              <button onClick={() => setMobileMenu(!mobileMenu)} className="md:hidden p-1.5 rounded-lg text-gray-400 hover:text-gray-200 hover:bg-gray-800/50 transition-all">
            ...
            <div className="max-w-7xl mx-auto px-4 py-3 grid grid-cols-3 gap-1">
              {NAV_MODES.map(mid => (
                <button key={mid} onClick={() => { setMode(mid); setMobileMenu(false) }}
                  className={`px-2 py-2 rounded-lg text-xs font-medium transition-all text-center ${mode === mid ? "bg-amber-500/10 text-amber-400" : "text-gray-400 hover:text-gray-200"}`}>
      ...
      <main className="flex-1 flex flex-col min-h-0 relative">
        {!showChat || messages.length === 0 ? (
          <>
            <section className="relative min-h-[85vh] flex items-center justify-center overflow-hidden">
              ...
              <div className="relative z-10 max-w-4xl mx-auto px-4 text-center py-16">
                ...
                <p className="text-base sm:text-lg text-gray-400 max-w-2xl mx-auto leading-relaxed mb-8">{t.desc}</p>
                ...
                <div className="flex justify-center gap-8 sm:gap-12 mt-12">
                  {[{ value: PLAYERS.length, label: t.stats[0], icon: "👤" }, { value: PLAYERS.length, label: t.stats[1], icon: "🏟️" }, { value: MODES.length, label: t.stats[2], icon: "🎯" }].map(s => (
                    <div key={s.label} className="text-center">
                      <div className="text-3xl sm:text-4xl font-black text-gray-100">{s.value}</div>
            ...
            <section id="modes" className="py-16 px-4 border-t border-gray-800/50">
              <div className="max-w-7xl mx-auto">
                <div className="text-center mb-10">
                  ...
                  <p className="text-sm text-gray-400">{t.modesDesc}</p>
                </div>
                  ...
                  {MODES.map(m => {
                    ...
                    return (
                      <button key={m.id} onClick={() => { setMode(m.id); setShowChat(true) }}
                        ...
                        <div className="text-sm font-semibold text-gray-100 mb-0.5">{m.name}</div>
            ...
            <section className="py-12 px-4 border-t border-gray-800/50 bg-gray-900/20">
              <div className="max-w-3xl mx-auto text-center">
                <p className="text-sm text-gray-400 mb-4">{lang === "es" ? "Prueba estas preguntas:" : "Try these questions:"}</p>
                <div className="flex flex-wrap gap-2 justify-center">
                  {(lang === "es"
                    ...
                    <button key={s} onClick={() => { setInput(s); setShowChat(true); setTimeout(() => inputRef.current?.focus(), 100) }}
                      className="px-3.5 py-2 text-xs rounded-lg bg-gray-800/50 text-gray-400 hover:text-gray-200 hover:bg-gray-700/50 transition-all border border-gray-700/50">{s}</button>
                  ))}
                </div>
              </div>
            </section>
            <footer className="border-t border-gray-800/50 bg-gray-950/80 py-8 px-4">
              <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-4 text-xs text-gray-600">
                  ...
                  <button onClick={() => { setShowChat(false); setMessages([]) }} className="hover:text-gray-400 transition-colors">{t.newChat}</button>
          ...
          <div className="flex-1 flex flex-col min-h-0">
            <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3 max-w-4xl mx-auto w-full">
              {messages.map((msg, i) => (
                <div key={i} className={`message-enter flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                  <div className={`max-w-[88%] sm:max-w-[72%] rounded-2xl px-4 py-3 ${msg.role === "user" ? "bg-amber-500/10 border border-amber-500/20 text-gray-100 rounded-br-sm" : "bg-gray-900/50 border border-gray-800/50 text-gray-300 rounded-bl-sm"}`}>
            ...
            <div className="flex-none border-t border-gray-800/50 bg-gray-950 px-4 py-3">
              <div className="max-w-4xl mx-auto flex gap-2.5 items-center">
                <div className="flex-1 relative flex items-center gap-2">
                  <input ref={inputRef} value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKeyDown}
                    placeholder={t.followUp}
                    className="flex-1 px-4 py-2.5 rounded-xl bg-gray-800/50 border border-gray-700/50 text-gray-100 placeholder-gray-500 focus:outline-none focus:border-amber-500/40 focus:bg-gray-800/80 text-sm transition-all"
                    disabled={loading} />
                  <button onClick={isListening ? stopListening : startListening}
                    className={`p-2.5 rounded-xl transition-all ${isListening ? "bg-red-500/20 text-red-400 border border-red-500/30 animate-pulse" : "bg-gray-800/50 text-gray-400 hover:text-gray-200 hover:bg-gray-700/50 border border-gray-700/50"}`}
